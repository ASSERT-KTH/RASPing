...
class Model:
    def __init__(self, raspFunction: rasp.SOp, inputs, seqLength: int, name: str):
        ...
        self.model = compiling.compile_rasp_to_model(
            self.raspFunction, self.inputs, self.seqLength, compiler_bos="BOS"
        )
        self.name = name
    ...
    def train(
        ...
        for epoch in tqdm.trange(n_epochs):
            ...
            if valStep and epoch % valStep == 0:
                ...
                if not returnAllMetrics:
                    ...
                    validations[0].append(
                        fastEvaluate(state.params, X_train, Y_train, padToken)
                    )
                    validations[1].append(val)
                    metrics[0].append(
                        loss_fn.apply(state.params, X_train, Y_train, padToken)
                    )
                    metrics[1].append(
                        loss_fn.apply(state.params, X_val, Y_val, padToken)
                    )
        ...
        if plot:
            ...
            if valStep:
                ...
                plt.plot(
                    np.linspace(0, n_epochs, len(validations)), [m for m in validations]
                )
                plt.xlabel("Epochs")
                plt.ylabel("Accuracy")
    ...
    def testOverTraining(
        ...
        for epoch in tqdm.trange(n_epochs):
            ...
            if valStep and epoch % valStep == 0:
                ...
                self.model.params = state.params
                print(
                    self.evaluateEncoded(X_val, Y_val, doPrint=False, outputArray=False)
                )
                    ...
                    for name1, _ in self.weightStatistics.items():
                        ...
                        for name2, weightStats in self.weightStatistics[name1].items():
                            ...
                            self.model.params[name1][name2] = self.model.params[name1][
                                ...
                                np.random.normal(0, param, layerShape),
                                0,
                            )
